...
export const useBagianStore = defineStore('bagian', {
  state: () => ({
    items: [],
    mores: [],
    meta: {},
  ...
  actions: {
    ...
    async getDataTable (more) {
      ...
      try {
        const resp = await api.get('/bagian', params)
    ...
    async deletesData (payload) {
      ...
      try {
        await api.post('/bagian/delete_data', params).then(resp => {
          console.log(resp)
        ...
        console.log('err bagian', error.response)
        notifErr(error.response)
      }
    },
    // setMores (payload) {
