...
export const useFormStore = defineStore('formpendidikan', {
  state: () => ({
    isOpen: false,
    form: {
      nama: '',
  ...
  actions: {
    ...
    async saveForm () {
      this.loading = true
      try {
        const resp = await api.post('/pendidikan/adding_data', this.form)
        console.log('save data', resp)
        if (resp.status === 201) { // ini untuk post request
          notifSuccess(resp)
          this.newData()
          // ini untuk panggil data table
          const jenisKepegawaianStore = usePendidikanStore()
