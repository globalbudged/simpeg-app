...
export const useFormStore = defineStore('formgolongan', {
  state: () => ({
    isOpen: false,
    form: {
      nama: '',
  ...
  actions: {
    ...
    async saveForm () {
      this.loading = true
      try {
        const resp = await api.post('/golongan/adding_data', this.form)
        console.log('save data', resp)
        if (resp.status === 201) { // ini untuk post request
          notifSuccess(resp)
          this.newData()
          // ini untuk panggil data table
          const golonganStore = useGolonganStore()
          golonganStore.getDataTable()
          this.loading = false
