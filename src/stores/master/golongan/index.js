...
export const useGolonganStore = defineStore('golongan', {
  state: () => ({
    items: [],
    mores: [],
    meta: {},
  ...
  actions: {
    ...
    async getDataTable (more) {
      ...
      try {
        const resp = await api.get('/golongan', params)
    ...
    async deletesData (payload) {
      ...
      try {
        await api.post('/golongan/delete_data', params).then(resp => {
          console.log(resp)
        ...
        console.log('err golongan', error.response)
        notifErr(error.response)
      }
    },
    // setMores (payload) {
