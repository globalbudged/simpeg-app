...
export const useJenisKepegawaianStore = defineStore('jeniskepegawaian', {
  state: () => ({
    items: [],
    mores: [],
    meta: {},
  ...
  actions: {
    ...
    async getDataTable (more) {
      ...
      try {
        const resp = await api.get('/jeniskepegawaian', params)
    ...
    async deletesData (payload) {
      ...
      try {
        await api.post('/jeniskepegawaian/delete_data', params).then(resp => {
          console.log(resp)
        ...
        console.log('err jeniskepegawaian', error.response)
        notifErr(error.response)
      }
    },
    // setMores (payload) {
