...
import { useJurusanStore } from 'src/stores/master/jurusan'
...
export const useFormStore = defineStore('formjurusan', {
  state: () => ({
    isOpen: false,
    form: {
      nama: '',
  ...
  actions: {
    ...
    async saveForm () {
      this.loading = true
      try {
        const resp = await api.post('/jurusan/adding_data', this.form)
    ...
    resetFORM () {
      this.form = {}
      const columns = ['nama', 'profesi', 'flag']
