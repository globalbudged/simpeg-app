<template>
    <div>
        <div class="bg-page q-pa-lg">
                ...
                <div class="col-md-7 col-xs-12 bg-page-secondary rounded-borders shadow-1">
                    <div class="q-pa-lg">
                        ...
                        <q-list bordered dense separator class="rounded-borders q-mb-lg" style="width:350px;">
                            <q-item v-for="(mutasi, i) in store.mutations" :key="i" tag="label" v-ripple>
            ...
            <div class="row q-my-md">
                ...
                <div class="col-md-7 col-xs-12">
                    <q-card>
                        <q-card-section>
                            <q-btn fab color="primary" icon="add" class="absolute" padding="xs"
                            ...
                            <div class="q-gutter-sm">
                                ...
                                <app-input v-if="store.jenis_kepegawaian.length < 1" outlined readonly
                                    icon="sentiment_dissatisfied" label="Pilihan Tidak Ada" />
                                <q-radio v-else v-for="(jenis, idx) in store.jenis_kepegawaian" :key="idx"
                                    v-model="store.form.jenis_kepegawaian_id" checked-icon="task_alt"
                                    unchecked-icon="panorama_fish_eye" :val="jenis.id"
                                    :label="`${jenis.nama} - ${jenis.kelompok}`"
                        ...
                        <q-card>
                                ...
                                <div class="q-mt-sm">
                                    <div class="row q-col-gutter-sm">
                                        <div class="col-md-4 col-xs-12">
                                            <app-input-date
                                                v-if="store.form.kode_mutasi==='MK' && store.form.jenis_kepegawaian_id===3"
                                                outlined :model="store.form.tgl_phk"
                                                @setModel="(value)=>store.setForm('tgl_phk', value)"
                                                label="Tanggal PHK" />
                                            <app-input-date v-else outlined :model="store.form.tgl_surat"
                                                @setModel="(value)=>store.setForm('tgl_surat', value)"
                                                label="Tanggal Surat" />
                                        </div>
                                        <div class="col-md-4 col-xs-12">
                                            <app-input-date outlined :model="store.form.tgl_mutasi"
                                                @setModel="(value)=>store.setForm('tgl_mutasi', value)"
                                                label="Tanggal Mutasi" />
                                        </div>
                                        <div class="col-md-4 col-xs-12">
                                            <app-input v-model="store.form.no_surat" outlined label="Nomor Surat" />
                                        </div>
                                        <div v-if="store.form.kode_mutasi=='MK' && store.form.jenis_kepegawaian_id==3"
                                            class="col-md-12 col-xs-12">
                                            <app-input
                                                :valid="store.form.kode_mutasi!=='MK' && store.form.jenis_kepegawaian_id!==3"
                                                v-model="store.form.dasar_phk" outlined label="Dasar PHK" />
                                        </div>
                                        <div v-if="store.form.kode_mutasi=='MK' && store.form.jenis_kepegawaian_id==3"
                                            class="col-md-12 col-xs-12">
                                            <q-radio v-for="(item, index) in store.jenis_phks" :key="index"
                                                v-model="store.form.jenis_phk"
                                                unchecked-icon="panorama_fish_eye"
                                                checked-icon="task_alt" :val="item.id" :label="item.nama"
                                                color="primary" />
                                        </div>
                                        <div v-if="store.form.kode_mutasi === 'MAR'" class="col-md-4 col-xs-12">
                                            <app-input :valid="store.form.kode_mutasi === 'MAR' ? false: true"
                                                v-model="store.form.dasar" outlined label="Dasar" />
                                        </div>
                                        <div v-if="store.form.kode_mutasi=='MAR'" class="col-md-4 col-xs-12">
                                            <app-input :valid="store.form.kode_mutasi === 'MAR' ? false: true"
                                                v-model="store.form.kepada" outlined label="Kepada" />
                                        </div>
                                        <div v-if="store.form.kode_mutasi=='MAR'" class="col-md-4 col-xs-12">
                                            <app-input :valid="store.form.kode_mutasi === 'MAR' ? false: true"
                                                v-model="store.form.untuk" outlined label="Untuk" />
                                        </div>
                                    </div>
                            ...
                            <q-card-section>
                                <div class="text-right">
                                    <app-btn :loading="store.loading" type="submit" color="primary"
        ...
        <AppDialogInput v-model="showAddJenis" @onSubmit="saveDataJenis" :loading="formJenis.loading">
